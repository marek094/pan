...
var pan = {
  ...
  init: function () {
    ...
    if (localStorage.panHidden == undefined) {
      localStorage.setItem('panHidden', '0');
    }

    pan.div.style.right = localStorage.panHidden == '1'
      ? kHIDDEN
      : kDISPLAYED;

    if (localStorage.selectedSection == undefined) {
      localStorage.setItem('selectedSection', 'units');
    }
  ...
  menu: {
    ...
    load: function () {
      this.dom().innerHTML = pan.sections()
        .map( ({title: t, name: n}) => t.charAt(0)
          ...
          .tagAs('a', {
            href: '#' + n,
            class: n + (n == localStorage.selectedSection ? ' active' : "")
          })
        )
        .join("")
        ;
    ...
    handler: function (target) {
      ...
      localStorage.selectedSection = target.className.split(/\s+/)[0];
  ...
  tabs: {
    ...
    units: {
      ...
      init: function () {
        ...
        // alert(Object.keys(pan.tabs.units));
        // alert(pan.tabs.units.content);
        pan.tabs.units.content.innerHTML = '<br>'
          + res
        ...
        [].slice.call(document.getElementsByClassName('UI')).forEach(x => {
    ...
    fights: {
      ...
      init: function () {
        ...
        pan.tabs.fights.content.innerHTML = Array.from(document.querySelectorAll('.unit'))
          .reduce( ([m, ps], u) => {
            ...
            const [,img, d] = match;
            const coor = [u.style.left, u.style.top].map( x => parseInt(x) / 64);
            const size = width / 64;
          ...
          .map(x=>x.tagAs('tr').tagAs('table', {class: 'fights'}))
          // .map(x=>tagAs('table class="fights"'))
          // .map(tagAs(['tr', 'table class="fights"']))
          .join("")
    ...
    post:  {
      ...
      init: function() {
        ...
        iframe.onload = function (e) {
          ...
          let submit = document.getElementById('odeslat');
          let input = document.getElementById('edit_player_name');
